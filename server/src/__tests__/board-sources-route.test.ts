...
describe("GET /api/companies/:companyId/board-authored-sources", () => {
  ...
  it("returns board-authored sources with default windowHours", async () => {
    ...
    expect(mockBoardSourcesList).toHaveBeenCalledWith("company-1", 24);
  });

  it("passes windowHours query parameter to service", async () => {
    const app = createApp(agentActor);
    const res = await request(app).get("/api/companies/company-1/board-authored-sources?windowHours=48");
